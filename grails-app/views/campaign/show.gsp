...
<div id="show-campaign" class="content scaffold-show" role="main">
    ...
    <ol class="property-list campaign">
        ...
        <g:if test="${campaignInstance?.sendDate}">
            <li class="fieldcontain">
                <span id="sendDate-label" class="property-label"><g:message code="campaign.sendDate.label"
                                                                            default="Send Date"/></span>

                <span class="property-value" aria-labelledby="sendDate-label"><g:formatDate
                        date="${campaignInstance?.sendDate}"/></span>

            </li>
        </g:if>

        <g:if test="${campaignInstance?.timeZone}">
            <li class="fieldcontain">
                <span id="timeZone-label" class="property-label"><g:message code="campaign.timeZone.label"
                                                                            default="Time Zone"/></span>

                <span class="property-value" aria-labelledby="timeZone-label"><g:fieldValue bean="${campaignInstance}"
                                                                                            field="timeZone"/></span>
        ...
        <g:if test="${campaignInstance?.app}">
            <li class="fieldcontain">
                ...
                <span class="property-value" aria-labelledby="app-label"><g:link controller="app" action="show"
                                                                                 id="${campaignInstance?.app?.id}">${campaignInstance?.app?.encodeAsHTML()}</g:link></span>
